...
export const MedicalRecordAccordion = ({ medicalRecord}) => {
    ...
    /* const downloadMedicalRecord = async (e) => {
        ...
        window.open(`http://localhost:3000/api/medical-records/${medicalRecord._id}/export`, '_self');
        // https://stackoverflow.com/questions/41938718/how-to-download-files-using-axios
    } */
    const downloadMedicalRecord = async (e) => {
        e.preventDefault();
        const response = await fetch(`http://localhost:3000/api/medical-records/${medicalRecord._id}/export`);
    ...
    const downloadPrescriptionRecord = async (id) => {
        ...
        const url = `${host}/api/medical-records/${medicalRecord._id}/prescriptions/${id}/export`;
                                            ...
                                            <Grid item xs={12} md={4} key={i}>
                                                <Paper elevation={1} sx={{ padding: 3 }}>
                                                    ...
                                                    <Avatar
                                                        ...
                                                        src={`https://elonkiptoo.s3.us-east-2.amazonaws.com/${item.physician_id.photo}`}
                                                        sx={{
                                                            width: 80,
                                                            height: 80,
                                                            bgcolor: blue[500],
        ...
        <ImageList sx={{ width: "100%", height: "80vh" }} cols={4} rowHeight={100}>
            {itemData?.map((item, i) => {
                ...
                let path = hasFileExtension(
                                ...
                                ? `https://elonkiptoo.s3.us-east-2.amazonaws.com/${item}`
                                :item;
                return (
                <ImageListItem key={i}>
                    <img
