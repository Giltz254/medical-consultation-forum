...
const Chat = () => {
  ...
  const handleSelectChatRoom = async (messages, id) => {
    //fetch all messages in that room
  ...
  const handleSubmit = async (e) => {
    ...
    setProcessing(true);
    //error validation
    if (message.message.length === 0 && message.images.length === 0)
      return false;
  ...
  useEffect(() => {
    ...
    socket
      .off("receive-chatrooms-update")
      .on("receive-chatrooms-update", async (chatrooms) => {
  ...
  return (
    <div>
      {chatRoomMessages?.length > 0 ? (
                    ...
                    primary={
                      message.sender._id === loggedInUser.user_id._id ? (
                        ...
                        <>
                          ...
                          <Box sx={{ display: "flex" }}>
                            <Avatar
                              alt={fullname(message?.sender.name)}
                              sx={{
                                width: 30,
                                height: 30,
                                alignSelf: "center",
                              }}
                              src={`https://elonkiptoo.s3.us-east-2.amazonaws.com/${message?.sender.photo}`}
                            />
                            {message.message && (
                              <Typography
                                className={
        ...
        <List className={classes.messageArea}>
          {selectedChatRoom
            ? `No Conversation Found ${fullname(hasChatRoom?.name) || ""}`
            : "Select a Chatroom"}
        </List>
      )}
      {/* <form noValidate> */}
...
const StandardImageList = ({ itemData }) => {
  ...
  return (
    <ImageList sx={{ width: "80%", height: 200 }} cols={3} rowHeight={100}>
      {itemData?.map((item, i) => {
        let path = hasFileExtension(item) ? `https://elonkiptoo.s3.us-east-2.amazonaws.com/${item}` : item;
        return (
          <ImageListItem key={i}>
            <img
              src={path}
